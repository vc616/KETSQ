...
def select(p):
    ...
    for i in range(len(day_sn)):
        ...
        if len(result) > 0:
            sq = "UPDATE `analysis_days` SET `日累计水量`='" + str(day_sum_list[i]/3600) + "' WHERE (`项目名称`='" + p + "') AND (`日序号`='" + str(day_sn[i]) + "') LIMIT 1"
            print(sq)
        else:
            sq = "INSERT INTO `analysis_days` (`项目名称`, `日序号`, `日累计水量`,`年月日`) VALUES ('" + p + "', '" + str(day_sn[i]) + "', '" + str(day_sum_list[i]/3600) + "','" + day_text[i] + "')"
            print(sq)
